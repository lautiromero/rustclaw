...
use anyhow::Result;
use rig::completion::{CompletionError, CompletionRequest, CompletionResponse, Message as RigMessage};
...
impl rig::completion::CompletionModel for CompletionModel {
    ...
    fn completion(
        ...
        async move {
            ...
            let body = build_nvidia_body(&self.model, &request)
                .map_err(|e| CompletionError::RequestError(
            ...
            if !resp.status().is_success() {
                ...
                return Err(CompletionError::ProviderError(format!("HTTP {}: {}", status, text)));
            }
            
            // 4. Parsear respuesta
            let nvidia_resp: NvidiaResponse = resp
                .json()
                .await
                .map_err(|e| CompletionError::RequestError(
                    std::io::Error::new(std::io::ErrorKind::Other, e.to_string()).into()
            ...
            let first_choice = nvidia_resp.choices.first()
            ...
            let choice_content = if let Some(tool_calls) = &first_choice.message.tool_calls {
                ...
                let tool_calls_rig: Vec<AssistantContent> = tool_calls.iter()
                    .filter_map(|tc| {
            ...
            tracing::debug!("📥 NVIDIA RESPONSE | id: {} | finish: {:?} | has_tool_calls: {}", 
                nvidia_resp.id,
    ...
    fn stream(
        ...
        async move {
            Err(CompletionError::ProviderError("Streaming not implemented for NVIDIA provider".into()))
        }
    }
}

// ==================== Helper: construir body con tool calling ====================
fn build_nvidia_body(model: &str, request: &CompletionRequest) -> Result<serde_json::Value, anyhow::Error> {
                ...
                for c in content.iter() {
                    match c {
                        ...
                        UserContent::ToolResult(tr) => {
                            ...
                            let result_text = tr.content.iter()
                                .filter_map(|tc| match tc {
                                    rig::message::ToolResultContent::Text(Text { text }) => Some(text.clone()),
                ...
                let text = content.iter()
                    .filter_map(|c| match c {
                        ...
                        AssistantContent::ToolCall(tc) => {
                            ...
                            tracing::debug!("🔧 AssistantContent::ToolCall detected: name={}, id={}", 
                                tc.function.name, tc.id);
                            None  // No incluir tool_calls en "content", van en campo separado
                        }
                        other => {
                            tracing::debug!("⚠️ AssistantContent no-text: {:?}", 
    ...
    // 🔍 LOG FINAL: ver cuántos mensajes y si hay vacíos
    ...
    if !request.tools.is_empty() {
        let tools_openai: Vec<serde_json::Value> = request.tools.iter()
            .map(|tool| {
                json!({
                    "type": "function",
                    "function": {
